...
fn main() {
    ...
    let entries: Vec<String> = (0..10000)
        .map(|i| {
            format!(
                "ERROR [service-{}] Database timeout - user:{} duration:{}ms",
                i % 10,
                i % 1000,
                (i % 5000) + 100
            )
    ...
    println!(
        "✅ Inserted {} entries in {:.2}ms",
        entries.len(),
        total_insert_time.as_millis()
    );
    println!(
        "   Average: {:.2}μs per insert",
